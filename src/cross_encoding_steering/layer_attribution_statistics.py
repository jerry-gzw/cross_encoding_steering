...
def build_layer_group_cluster_statistics(
    ...
    *,
    ...
    depth_modes = [
        ...
        if mode in set(grouped["mode"].astype(str))
    ]
    depth_pairs = ((0.75, 0.5), (0.875, 0.5), (0.875, 0.625))
    for mode in depth_modes:
        mode_frame = grouped.loc[
            grouped["mode"].eq(mode)
        ...
        for metric, value in (
            ...
            metric_frame = mode_frame.loc[mode_frame[value].notna()].copy()
            index = [
                "model_alias",
                "pair_type",
                "mapping_name",
                ...
                contrast_rows.append(
                    {
                        ...
                        "confidence": confidence,
                        "inference_unit": (
                            "paired depth difference with "
                            "setting_behavior_group cluster resampling"
                        ),
                    }
                )
